    ...
    new Vue({
      ...
      methods: {
        ...
        this.nomes = [];
          ...
          axios
            .get('http://localhost:8080/chassiBoletim/findBoletim?idChassi=' + this.idChassi)
            .then(response => {
        ...
        const vm = this; // Armazena uma referência ao objeto Vue
        if (event.target.checked) {
          vm.incorporated = true;
          console.log('O checkbox está marcado');
        } else {
          vm.incorporated = false;
          console.log('O checkbox está desmarcado');
        }
        console.log("Ta true? " + vm.incorporated);
        this.search();
      }
      ,
      applicableFunction(event) {
        ...
        console.log("Ta true? " + vm.applicable);
        this.search();
      },
        save() {
        ...
        alter() {
          ...
          axios.post('http://localhost:8080/chassiBoletim/saveChassi', this.tableData)
            .then(response => {
